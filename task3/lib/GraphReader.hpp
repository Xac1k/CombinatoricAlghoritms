...
private:
    ...
    Graph<T> ConvertStringViewToGraph(const AgileMatrix<std::string>& matrix, const Parser<T>& parser);
public:
    GraphReader() = default;
    ...
    Graph<T> ReadFromFile(const std::string& rpath, const Parser<T>& parser);
};
...
Graph<T> GraphReader<T>::ReadFromFile(const std::string& rpath, const Parser<T>& parser) {
    auto tmpMatrix = ParseFileToStrMatrix(rpath);
...
bool GraphReader<T>::IsSquareMatrix(const AgileMatrix<std::string>& matrix) {
    size_t sizeY = matrix.size();

    for(int y = 0; y < sizeY; y++) {
        if(matrix[y].size() != sizeY) 
            return false;
    }
    return true;
}