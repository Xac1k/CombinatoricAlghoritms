...
std::tuple<std::vector<int>, float> getPlacesWithMinSum(Graph& cost, Graph& demanding) {
	...
	do
	{
		...
		for (int id_i = 0; id_i < cost.sizeX; id_i++) {
			for (int id_j = 0; id_j < cost.sizeX; id_j++) {
				int count = demanding.getWeight(id_i, id_j);
				int price = cost.getWeight(ManufactureID_PlaceID[id_i], ManufactureID_PlaceID[id_j]);
	...
	return std::make_tuple(minPlacing, sumOfPlacing);
}
...
int main()
{
	try {
		...
		std::cout << "Enter relative path to file CostOfDeliveringOrders" << std::endl;
		...
		timer.start();
		auto [places, cost] = getPlacesWithMinSum(graphOfCost, graphOfDemanding);
		timer.end();
		timer.printDuration("Time of producing: ");
