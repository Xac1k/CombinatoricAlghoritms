...
constexpr uint32_t GREEN = 255 << 8;
constexpr uint32_t EGGPLANT = 0x5b1c4e;
    ...
    win.Subscribe<KeyPressEvent>([&](const KeyPressEvent& e) {
        if (e.keyCode == sf::Keyboard::C) {
            ...
            if (terminals.size() < 2) {}
                ...
                win.DrawLine(terminals.at(0).second, terminals.at(1).second, BLACK, 1);
            }
            else {
                ...
                for (const auto& edge : graph->edges)
                    win.DrawLine(edge.GetStartPoint(), edge.GetEndPoint(), BLUE, 1);
