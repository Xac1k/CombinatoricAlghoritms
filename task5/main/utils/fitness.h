...
inline double Fitness(const Individual& individual,const std::vector<Item>& characteristics, const double capacity) {
    ...
    const double value = maxPair * ErrorFunc(generalVolume, capacity);
    const double pen = value;
...
inline CountIndividualsNextGeneration ComputeNumberOfIndividualsOnNextGen(const Population& population, const FitnessVect& fitnessVect) {
